<x-app-layout>
    <div class="max-w-4xl mx-auto py-6">
        ...
        <div class="mt-4">
            ...
            <select name="lesson" id="lesson-select" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                <option value="" disabled selected>Selecteaza un curs</option>
                @foreach($lessons as $index => $lesson)
                    @php
                        $subject = \App\Models\Subject::find($lesson['subjectid']);
                        $subjectName = $subject ? $subject->name : 'N/A';
                        $teacherids = \App\Models\Teacher::find($lesson['teacherids']);
                        $teacherName = $teacherids ? str_replace('_', ' ', $teacherids->name) : 'N/A';
                        $classids = \App\Models\Clas::find($lesson['classids']);
                        $className = $classids ? $classids->name : 'N/A';
                    @endphp
                    @if($className !== 'N/A') {{-- Only show options where class name is not N/A --}}
                    <option value="{{$lesson['id']}}" {{$index == 0 ? 'selected' : ''}} data-lesson="{{ $subjectName }}" data-teacher="{{ $teacherName }}" data-class="{{ $className }}">
        ...
        <div class="mt-6">
            <h2 class="text-lg leading-6 font-medium text-gray-900">Curs: <span id="selected-lesson" class="text-indigo-600">N/A</span></h2>
            <h2 class="text-lg leading-6 font-medium text-gray-900">Profesor: <span id="selected-teacher" class="text-indigo-600">N/A</span></h2>
            <h2 class="text-lg leading-6 font-medium text-gray-900">Grupa: <span id="selected-class" class="text-indigo-600">N/A</span></h2>
        </div>
    </div>
    ...
    <script>
        ...
        if(select.options.length > 1 && select.options[1].dataset) {
            const initialData = select.options[1].dataset;
